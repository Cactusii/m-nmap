...
#include <stdio.h>
#include <iostream>
std::map<int, std::map<std::string, std::string> > *port_probe_resp = new std::map<int, std::map<std::string, std::string> >();
...
// This function consults the ServiceGroup to determine whether any more probes can be launched at this time.  
// If so, it determines the appropriate ones and then starts them up.
// 此函数会咨询 ServiceGroup 以确定此时是否可以启动更多探测器。 如果是这样，它会确定适当的，然后启动它们。
...
static void servicescan_connect_handler(nsock_pool nsp, nsock_event nse, void *mydata) {
  ...
  if (svc->target->timedOut(nsock_gettimeofday())) {
    ...
    // 是的！ 连接到端口。 发送适当的探针文本（如果需要的话——可能是 NULL 探针）
    svc->currentprobe_exec_time = *nsock_gettimeofday();
    send_probe_text(nsp, nsi, svc, probe);
    // Now let us read any results
    // 读取返回结果
    ...
    if (MD && MD->serviceName) {
      ...
      if (MD->isSoft && svc->probe_matched) {
        ...
        // it will probably still match.
          ...
          // lmx
          std::string resp((char *)readstr, readstrlen);

          int probestringlen;
          const u8 *text = probe->getProbeString(&probestringlen);
          std::string probe_text((char *)text, probestringlen);

          (*port_probe_resp)[svc->portno][probe_text] = resp;
...
std::list<ServiceNFO *>::iterator svc;
 ...
 for(svc = SG->services_finished.begin(); svc != SG->services_finished.end(); svc++) {
   ...
   log_write(LOG_STDOUT, "[Service fingerprint]\n%s\n", (*svc)->getServiceFingerprint(NULL));
   int readstrlen = 0;
   u8* readstr  = (*svc)->getcurrentproberesponse(&readstrlen);
   log_write(LOG_STDOUT, "[Service proberesponse]\n");
   for(int i = 0; i < readstrlen; i++)
       log_write(LOG_STDOUT, "%c", *(readstr + i));
   log_write(LOG_STDOUT, "\n********************************************\n\n");

   for (std::map<int, std::map<std::string, std::string> >::iterator port = (*port_probe_resp).begin(); port != (*port_probe_resp).end(); port++)
   {
       log_write(LOG_STDOUT, "********** port:%5d **********\n", port->first);
       for (std::map<std::string, std::string>::iterator probe = port->second.begin(); probe != port->second.end(); probe++)
       {
           std::cout << "probe: " << probe->first << std::endl;
           std::cout << "resp:" << probe->second << std::endl;
       }
       log_write(LOG_STDOUT, "********************************\n");
   }
 }
